...
interface InterviewScheduleBody {
...
function validateBody(body: InterviewScheduleBody): string | null {
  const { candidateName, advisorName, interviewMethod, email, slot1 } = body;

  if (!candidateName?.trim() || !advisorName?.trim() || !interviewMethod?.trim()) {
    return "必須項目が不足しています";
  }
...
function buildLineWorksMessage(b: InterviewScheduleBody): string {
  return [
    "📅 面接希望日が届きました",
    "",
    "■ 担当アドバイザー",
...
function buildCandidateHtml(b: InterviewScheduleBody): string {
  return `
<div style="font-family:'Helvetica Neue',Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;">
  <p>${b.candidateName} 様</p>
  <p>この度は面接の希望日をお送りいただき、誠にありがとうございます。<br>以下の内容で受け付けいたしました。</p>
  <div style="background:#f5f5f5;padding:16px;border-radius:8px;margin:20px 0;">
    <p style="margin:0 0 8px;"><strong>■ 面接方式</strong><br>${b.interviewMethod}</p>
...
export async function POST(request: Request) {
  try {
    const body: InterviewScheduleBody = await request.json();
    ...
    const emailPromise = (async () => {
      ...
      try {
        ...
        await resend.emails.send({
          ...
          subject: "【株式会社ビズスタジオ】面接希望日を受け付けました",
    ...
    const portalTaskPromise = createPortalTask({
      ...
      meetingFormat: body.interviewMethod,
